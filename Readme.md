...
To update the API you can locally execute `dl-swagger.sh` to retrieve the latest swagger definitions for all services and commit them. To test the generated API definition on your local system you can use `local-api-generation.sh`. It will do the same steps as our GitHub Action. After it completes you can [run the MKDocs][#run-mkdocs] and see the generated APIs.
